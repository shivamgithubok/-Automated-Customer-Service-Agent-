    ...
    #     # print(f"Received question hi there: {question}")
    #     try:
    ...
    #         # Create prompt template with examples and legal format
    #         prompt_template = PromptTemplate(
    #             input_variables=["question", "context"],
    #             template="""
            #         You are a helpful assistant. Analyze the question type and provide a focused response based on the context. 
            #         The document contains several sections, each with a title. Please identify the sections, and answer based on the relevant section.
            #         -------------------------------------------------------
            #         ## Example Question:
            ...
            #         -------------------------------------------------------
            #         Instructions:
            #         1. First identify if this is a Resume or Legal Document
            #         2. Use the appropriate format based on document type
            #         3. Extract relevant information from the context
            #         4. Format response according to the matching template
            #     """
            # )

    #         # Generate prompt with all variables
...
import hashlib
# import uuid
# from agents.prompts.agent_prompt import Legal_Document
# from agents.prompts.resume_type import examples
...
class RAGAgent:
    ...
    def query(self, question: str, k: int = 2) -> dict:
        """Query the vector store and generate a response with context and justification."""
        try:
            print("Entering try block")  # Debug
            
            if not self.vector_store:
                print("No vector store found")  # Debug
                return {"error": "No documents have been processed yet"}
            
            print("Starting similarity search...")  # Debug
            relevant_docs = self.vector_store.similarity_search(question, k=k)
            print(f"Found {len(relevant_docs)} relevant documents.")  # Debug
            
            if not relevant_docs:
                return {"answer": "No relevant documents found.", "justification": "", "context": "", "source_documents": []}

            context = "\n".join([doc.page_content[:] + "..." for doc in relevant_docs])
            print("Context prepared.")  # Debug
            ...
            print("Prompt generated.")  # Debug

            # Get response from LLM
            response = llm.invoke(prompt)
            print("Response from LLM received.")  # Debug
            answer = response.content.strip()
            ...
            return {
                ...
                "justification": justification,
                "context": context,
                "source_documents": [doc.page_content[:30] for doc in relevant_docs[:1]],
                "num_docs_found": len(relevant_docs)
            }
